...
class WindowedFile:
    ...
    def open_file(self, path: str):
        ...
        if file_path == self.last_opened_file:
            pass  # Keep the old window_start position
        else:
    ...
    def go_to(self, line_number: int):
        if not self.file_lines:
            return "Error: 3No file is currently opened."
    ...
    def scroll(self, direction: str):
        if not self.file_lines:
            return "Error: 2No file is currently opened."
    ...
    def get_current_window(self):
        if not self.file_lines:
            return "Error: 1No file is currently opened."

...
def extract_file_diffs(git_show_string):
    """
    Extracts per-file diffs from a raw `git show` string.
    For each file, includes all lines starting from the first '@@' line
    (including the '@@' line itself), and returns them in a dictionary
    keyed by the filename (using the 'b/' side as canonical).

    Args:
        git_show_string (str): A `git show` diff string that may contain multiple file diffs.

    Returns:
        dict: A mapping of filenames (str) to their corresponding diff content (str),
              each diff starting from the first '@@' hunk header.
    """
    ...
    for line in lines:
        if line.startswith("diff --git"):
            if current_diff and current_file:
                result_diffs[current_file] = "\n".join(current_diff).strip()
            current_diff = []
            at_at_found = False
